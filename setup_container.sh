...
docker kill opengl_dev_env_p2 2> /dev/null
docker container rm opengl_dev_env_p2 2> /dev/null

printf "${YELLOW}\n⏱️ Running the container...${NORMAL}\n\n"

docker run -p 25901:5901 --mount type=bind,source="${PWD}/src",target=/src_code/ -d --rm -e "TERM=xterm-256color" --name opengl_dev_env_p2 vnc_ubutnu_xfce;

CONTAINER_IP=`docker inspect opengl_dev_env_p2 | grep "                    \"IPAddress\":" | cut -d \" -f 4`
...
docker exec opengl_dev_env_p2 bash -c "echo 'export \"PS1=\[\033[01;32m\]\u@\h\[\033[00m\]:\[\033[01;34m\]\w\[\033[00m\]\$ \"' >> ~/.bashrc"

docker exec -it -w /src_code opengl_dev_env_p2 bash
